...
# , "location_score", "academics_score", "finance_score", "other_score")
CARD_COLUMNS = ("university_id", "name", "city", "state",
                "website", "campus_location", "total_enrollment")
UNI_COLUMNS = ("university_id", "name", "city", "state", "website", "campus_location", "total_enrollment", "control", "religious", "accepts_ap_credit", "study_abroad", "offers_rotc", "has_football", "has_basketball", "ncaa_member", "retention_rate", "graduation_rate", "total_applicants", "total_admitted", "admission_rate", "male_applicants", "female_applicants", "male_admitted",
               "female_admitted", "sat_rw_25", "sat_rw_75", "sat_math_25", "sat_math_75", "act_25", "act_75", "in_state_price", "out_of_state_price", "average_price_after_aid", "percent_given_aid", "percent_american_indian_native_alaskan", "percent_asian", "percent_hawaiian_pacific_islander", "percent_black", "percent_white", "percent_hispanic", "percent_other", "percent_two_races")
...
def create_account():
    if request.method == "POST":
        ...
        cur.execute("SELECT * from login_information where username = %(new_username)s",
    ...
    if request.method == "POST":
        ...
        if form_details['submit_button'] == 'update':
            ...
            if len(login_exists) == 0:
                ...
                cur.execute("UPDATE login_information SET password=%(new_password)s WHERE username=%(username)s", {
                            'new_password': new_password, 'username': session["username"]})
                mysql.connection.commit()
                cur.close()
        ...
        # Delete Account
            ...
            if len(login_exists) == 0:
                ...
                cur.execute("DELETE FROM login_information where username=%(username)s and password=%(password)s", {
                            'username': session['username'], 'password': password})
                mysql.connection.commit()
                cur.close()
...
def find_colleges():
    ...
    if request.method == "POST":
        parameters = request.form
        ranks = ['location_rank', 'academics_rank',
                 'finance_rank', 'other_rank']
...
def saved_universities():
    ...
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("DELETE FROM saved_universities WHERE user = %(username)s and university_id = %(id)s;", {
                    'username': session['username'], 'id': university['delete']})
        mysql.connection.commit()
        cur.close()
...
def college_info(id):
    ...
    if len(data):
        ...
        majors = [major[0].strip().replace('"', '')
                  for major in cur.fetchall()]
...
def find_majors():
    if request.method == "POST":
        ...
        if majors_list:
            ...
            cur = mysql.connection.cursor()
            cur.execute(
                "SELECT * FROM user_major_type wHERE user = %s", [session['username']])
            results = cur.fetchall()

            if len(results) == 0:
                print(results)
                cur.execute("INSERT INTO user_major_type(user, major_type) VALUES(%s, %s)",
                            (session['username'], major_type))
            else:
                cur.execute("UPDATE user_major_type SET major_type=%s WHERE user=%s",
                            (major_type, session['username']))
            mysql.connection.commit()
